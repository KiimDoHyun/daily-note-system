...
이 문서는 이 리포지토리에서 코드를 읽거나 수정하는 LLM(Claude 등) 이 먼저 읽어야 할 안내다. 사람용 문서는 [README.md](README.md), 설계 상세는 [docs/design.md](docs/design.md) 를 본다.

---

## 1. 이 프로젝트가 하는 일

옵시디언 볼트에 매일 하나씩 데일리 노트를 생성하고, 어제 노트의 미완료 항목을 오늘로 이월시키며, 오래된 항목은 자동 드롭·보관하는 macOS 자동화 시스템이다.
...
옵시디언에 종속되지 않는다. 마크다운 파일과 폴더 구조만 다룬다. `[[wikilink]]` 하나만 옵시디언 전용.
...
macOS TCC 는 다음 폴더를 잠근다.

- `~/Documents/`, `~/Desktop/`, `~/Downloads/`, `~/Library/Mobile Documents/`

이 안에 리포가 놓이면 launchd 로 뜨는 파이썬이 **스크립트 파일을 여는 순간부터** 거절당한다. 부팅 직후엔 특히 심하다. `install.sh` 가 이 위치를 검사해서 중단하지만, 수동 배치 시엔 반드시 홈 폴더 바로 아래(`~/daily-note-system/` 등)를 쓴다.

이 규칙은 실제 사고에서 학습된 것이다. 초기 배치를 `~/Documents/workspace/daily-note-system/` 에 했다가 부팅 직후 자동 실행이 계속 실패했다. 배경 상세는 이 파일 하단 "부록 A" 참고.

### 3.2 config.py 를 doctor 에서 참조하지 말 것

`lib/doctor.py` 는 **`DAILY_NOTE_VAULT_ROOT` 미설정 상태에서도 돌아야 한다.** `lib.config` 는 로드 즉시 이 환경 변수를 요구하므로 doctor 에서 import 하면 크래시한다. doctor 는 os.environ 을 직접 읽는다.

같은 이유로 `daily_note.py` 의 `_log`, `_log_error` 는 `lib.config` 를 함수 내부에서 지연 import 한다. 최상위 import 로 되돌리지 말 것.

### 3.3 외부 의존성 추가 금지

macOS 표준(Python 3.9 + 시스템 라이브러리 + bash + osascript) 만 사용한다. `pip install`, Homebrew 도구, 서드파티 라이브러리 추가는 배포 마찰을 만든다. 알림도 `osascript` 로 처리한다 (`terminal-notifier` 등 도입 금지).

### 3.4 launchd 우회 대안 도입 금지

launchd 대신 cron, `at`, 서드파티 스케줄러 등을 섞지 말 것. macOS 표준 방식으로 통일. cross-platform 확장은 별도 리포에서 다룬다.

### 3.5 진단 억제 금지

`doctor` 나 `install.sh` 의 실패 조건을 완화·비활성화하는 방향의 수정은 하지 말 것. 실패가 나는 이유가 있고, 그 실패를 조기에 잡는 것이 이 시스템의 방어 계층 중 하나다.
...
- 스크립트 상수 하드코딩 지양.
- Finder 대기 로직을 삭제하지 말 것. 이건 부팅 직후 TCC 데몬이 준비될 시간을 벌기 위한 것이지, 순수 지연이 아니다.
...
87개 이상이어야 한다 (2026-09-16 기준). 신규 기능은 반드시 테스트를 함께 추가.
...
- 커밋 메시지는 영어. 대문자 명령형 동사로 시작 (`Add`, `Improve`, `Fix`, `Refactor`, `Document`). 기존 히스토리 스타일을 따른다.
- 테스트가 깨진 상태로 커밋 금지.
- README 는 사용자용, CLAUDE.md 는 LLM/미래의 자기 자신용. 서로 중복 최소화, 대신 상호 링크.
...
**옛 셸 스크립트 시절엔 왜 안 걸렸나**: 옛 스크립트는 `~/.claude/scripts/` (잠긴 폴더 밖) 에 있어서 bash 가 스크립트 파일을 여는 순간이 TCC 검사 대상이 아니었다. 볼트 접근은 bash 의 오래된 TCC 캐시로 통과. 새 파이썬 스크립트는 `~/Documents/workspace/` (잠긴 폴더 안) 에 있어서 스크립트 파일 자체를 여는 첫 단계부터 TCC 대상이 되었고 부팅 직후에 걸렸다.

**결정적 변수는 "스크립트 위치"**. 그래서 이 프로젝트는 다음 세 층으로 방어한다.
...
이 세 층 중 하나만 있어도 대부분 막히지만, 셋 다 있는 이유는 각각이 다른 사고 시나리오를 잡기 때문이다. 방어층을 임의로 제거하지 말 것.
